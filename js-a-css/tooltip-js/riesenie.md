...
Vytvorte skript, ktorý zobrazí doplňujuci text pokiaľ používateľ umiestni kurzor nad `<span>` elementy v dokumente.
...
```
Kód obsahuje `<div>`, ktorého obsah je tvorený textom generovaný pomocou [Lorem ipsum generátora](https://www.lipsum.com/). V tomto texte sa ďalej nachádzajú elementy `<span>`, kde prvý span označuje výraz a druhý `<span>` s triedou `tooltip` obsahuje text doplňujúceho textu, ktorý sa ma používateľovi zobraziť ak nad prvý `<span>` umiestni kurzor. Akonáhle používateľ dá kurzor z prvého `<span>` preč, doplňujúci text sa skryje. Samozrejme, pri otvorení dokumentu nesmú byť doplňujúce texty zobrazené.
